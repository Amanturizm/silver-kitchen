...
const Brands = () => {
  ...
  return brands?.length && (
    <div className="container relative">
      ...
      <div
        className="absolute right-0 top-1/2 -translate-y-1/2 bg-white rounded-[20px] py-[30px] w-[40%] flex items-center gap-4"
      >
        <button
          onClick={() => scroll('left')}
        ...
        <div
          ref={sliderRef}
          className="flex overflow-hidden scroll-smooth flex-1"
        >
          {brands.map((brand) => (
            <div
              key={brand.id}
        ...
        >
          <Image src={arrowLeftIcon} alt="arrow-right" width={24} height={24} className="rotate-180"/>
        </button>
      </div>
    </div>
  );
